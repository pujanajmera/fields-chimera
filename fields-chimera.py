    ...
    with open(file_path, 'r') as file:
        for line in file:
            if line.startswith('#Sample Density'):
                ...
                try:
                    basis_matrix.append([float(x) for x in line.split()[1:4]])
                except:
...
def transform_field(field_array, center_array, basis_matrix_array):
    ...
    transformed_field_coords = np.matmul(field_coords, basis_matrix_array.T)
...
def generate_tip_tail_vectors(field_array, sample_density_array, volume_box_array):
    ...
    Inputs:
        field_array: Field array (transformed or not)
        sample_density_array: Sample density array
    ...
    if sample_density_array!=[] and volume_box_array!=[]:
        max_vector_length = 0.5*np.min(volume_box_array/sample_density_array)
    else:
    ...
    tip_to_tail_vectors = np.concatenate((tip_vecs, tail_vecs), axis=1)

    return tip_to_tail_vectors

def generate_bild_file(tip_to_tail_vectors, transformed_field_array, chimera_type):
    '''
    ...
    r = field_mags
    g = field_mags
    b = np.ones((field_mags.shape[0], 1))
...
def main():
    ...
    generate_bild_file(tip_to_tail_vectors, transformed_field_array, args.ch)
